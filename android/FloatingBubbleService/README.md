...
- The service ignores very low RMS/silent captures before sending them to the backend, so the user gets a fast local message when the video is muted or the app blocks capture.
- Repeated taps are rate-limited briefly to avoid overlapping audio uploads and overheating.
...
- The backend endpoint is `/api/audio-translate`; it accepts a short WAV clip as base64, rejects oversized payloads, and returns `{ transcript, arabic, confidence }`.
