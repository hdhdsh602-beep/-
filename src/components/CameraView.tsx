...
export default function CameraView({ 
  ...
  const processLiveTextFrame = async (video: HTMLVideoElement, force = false) => {
    if (liveOcrBusyRef.current || !tesseractReadyRef.current || (!force && ocrScanning) || document.visibilityState !== 'visible') return;
      ...
      try {
        {
          ...
          if (signAutoTranslateEnabled && !ocrScanning && liveOcrReady && now - lastLiveOcrFrameRef.current > LIVE_OCR_INTERVAL_MS) {
  ...
  }, [modelReady, cameraActive, focusedObject, focusProgress, glassesFilterMode, activeViewMode, accuracyThreshold, liveOcrReady, signAutoTranslateEnabled, ocrScanning, isOfflineModeActive, autoSpeak]);
